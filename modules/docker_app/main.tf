...
resource "null_resource" "wait_healthy" {
  ...
  provisioner "remote-exec" {
    inline = [
      "echo 'Waiting for ${var.app_name} to be healthy...'",
      "for i in $(seq 1 ${var.health_check_retries}); do",
      "  status=$(docker inspect --format='{{.State.Health.Status}}' ${var.app_name} 2>/dev/null || echo 'starting')",
      "  if [ \"$status\" = \"healthy\" ]; then",
      "    echo '${var.app_name} is healthy!'",
      "    exit 0",
      "  fi",
      "  echo \"Attempt $i: status=$status, waiting...\"",
      "  sleep ${var.health_check_interval}",
      "done",
      "echo 'Health check timeout for ${var.app_name}'",
      "exit 1"
    ]
  }
}
